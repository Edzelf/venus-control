...
bool homewizard_handle()
{
  ...
  if ( read_p1_dongle_http() )                                  // Lees JSON van P1 via http
  {
    ...
    p1 = json_doc["active_power_w"] ;                           // Bepaal netto vermogen (grid naar huis)
  ...
  {
    #ifdef TESTING
      jsonerr = deserializeJson ( json_doc, testjson ) ;
      if ( jsonerr != DeserializationError::Ok )
      {
        dbgprint ( "P1 JSON codering fout %d, lengte is %d!",
                   jsonerr, strlen ( testjson ) ) ;
      }
      else
      {
        dbgprint ( "P1 Json lengte is %d gelukt", strlen ( testjson ) ) ;
        int32_t p1 = json_doc["active_power_w"] ;
        dbgprint ( "active_power_w is %d", p1 ) ;
      }
    #endif
  }
  return false ;
